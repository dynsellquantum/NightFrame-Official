...
`NIGHTFRAME.sln` also contains Windows-only legacy projects and should be built
on Windows.
...
Release signing requires a private `nightframe.keystore` and the
`NIGHTFRAME_KEYSTORE_PASS` environment variable. Neither should be committed.
